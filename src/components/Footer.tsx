            ...
            <div className="col-span-2 md:col-span-1">
              <div className="flex items-center gap-2.5 mb-4">
                ...
                <div>
                  ...
                  <span className="block text-saffron-500 text-[10px] font-semibold">v6.1.3</span>
                </div>
              </div>
              ...
              <div className="flex gap-2">
                {socials.map((s, i) => (
                  <a key={i} href={s.url} aria-label={s.name} target="_blank" rel="noopener noreferrer"
                    className={`w-9 h-9 rounded-xl flex items-center justify-center bg-slate-800/80 text-slate-400 hover:bg-slate-800 border border-slate-700/50 transition-all duration-300 hover:scale-110 ${s.color}`}>
                    {s.icon}
                  </a>
                ))}
              </div>
        ...
        <div className="border-t border-slate-800/60 py-6 sm:py-8">
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            <p className="text-slate-500 text-xs sm:text-sm text-center sm:text-left">
              © {currentYear} <span className="sinhala-text text-saffron-500/70">සිංහල පුළුවන්ද?</span> — All rights reserved. Made by Amantha I. Salgadu.
            </p>
