...
export default function AboutSection({ darkMode }: AboutSectionProps) {
  return (
    <section id="about" className={`py-16 sm:py-24 px-5 sm:px-6 relative overflow-hidden ${darkMode ? 'bg-slate-950' : 'bg-gradient-to-b from-white to-slate-50'}`}>
      ...
      <div className={`absolute bottom-0 left-0 w-80 h-80 rounded-full blur-[120px] ${darkMode ? 'bg-purple-500/5' : 'bg-purple-500/8'}`} />
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-5">
            {FEATURES.map((f, i) => (
              <div key={i}
                ...
                <p className={`text-[10px] sm:text-xs leading-relaxed ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
